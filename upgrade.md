...
- Le template est dans codetovecto.config.js
...
- output.js doit exposer une fonction (module.exports) qui choisit JSON vs chroma selon config.output et écrit effectivement les fichiers.

2) Gestion d’erreurs réseau
- Dans ai.js : ajouter try/catch, timeout, et gestion des erreurs (429, 5xx).
...
Problème observé : certaines analyses retournent du texte incohérent/bruité (hallucinations, mélange de langues, contenu “meta”).
Améliorations :
- Encadrer strictement l’entrée (délimiteurs, instructions courtes, pas de “décris la page” pour un fichier backend).
- Forcer une sortie structurée JSON (schema stable) et valider le JSON (sinon retry).
...
- Ajouter une étape chunking (découper en segments avec metadata : file, startLine/endLine, type).
